...
/* \function igraph_community_leiden_overlapping
 * \brief Overlapping community detection with the Leiden-CPM algorithm.
 *
 * Detects overlapping (vectorized) communities by optimizing the
 * overlapping Constant Potts Model described in the section comment
 * above: each node may belong to up to \p max_memberships communities
 * with intensity 1/sqrt(k), pairs of nodes interact with coefficient
 * |sigma_i n sigma_j| / sqrt(k_i k_j) <= 1 (no edge double-counting), and
 * the local moving phase plays an exact potential game whose unilateral
 * actions are membership additions, removals and substitutions. On covers
 * where every node holds a single membership, the objective coincides
 * with the CPM optimized by \ref igraph_community_leiden().
 *
 * \param graph The input graph. It must be undirected.
 * \param edge_weights Numeric vector of edge weights, or \c NULL for
 *    weight 1 everywhere.
 * \param node_weights Numeric vector of node weights, or \c NULL for
 *    weight 1 everywhere.
 * \param resolution_parameter The resolution parameter gamma.
 * \param beta Randomness used in the refinement step (as in
 ...
 * \return Error code.
 *
 * Time complexity: roughly that of the Leiden algorithm run on a graph
 * with n*K vertices and m*K^2 edges.
 */
